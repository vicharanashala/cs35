...
function DashboardTab() {
  ...
  const { data: categoryStats = [] } = useQuery({ queryKey: ["category-stats"], queryFn: categoryApi.getStats });

  const COLORS = ["#4F46E5", "#D97706", "#16A34A", "#9333EA", "#0D9488", "#EC4899", "#F97316", "#06B6D4"];

  const totalItems = useMemo(() => {
    return categoryStats.reduce((acc, curr) => acc + curr.questionCount + curr.faqCount, 0);
  }, [categoryStats]);

  const sortedStats = useMemo(() => {
    return [...categoryStats].sort((a, b) => (b.questionCount + b.faqCount) - (a.questionCount + a.faqCount));
  }, [categoryStats]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Topic Category Distribution (Option B) */}
        <div className="card p-6 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 mb-6">
              <span className="text-lg">📊</span>
              <h2 className="font-bold text-base" style={{ color: "#1F2937" }}>Topic Category Distribution</h2>
            </div>
            {totalItems === 0 ? (
              <div className="text-center py-10 text-sm text-gray-400">
                No question or FAQ data available to show category distribution.
              </div>
            ) : (
                ...
                <div className="relative shrink-0" style={{ width: size, height: size }}>
                  <svg width={size} height={size} className="transform -rotate-90">
                    ...
                    {sortedStats.map((c, index) => {
                      const count = c.questionCount + c.faqCount;
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-xl font-extrabold text-slate-800">{totalItems}</span>
                    <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">Total Items</span>
                  </div>
                </div>

                {/* Progress Bars / Legend */}
                <div className="flex-1 w-full space-y-3">
                  {sortedStats.slice(0, 5).map((c, i) => {
                    const count = c.questionCount + c.faqCount;
                    const pct = totalItems > 0 ? Math.round((count / totalItems) * 100) : 0;
                    return (
                      <div key={c.name} className="space-y-1">
                        <div className="flex justify-between text-xs font-semibold">
                          <span style={{ color: "#374151" }} className="flex items-center gap-1.5 truncate max-w-[150px]">
        ...
        <div className="card p-6 flex flex-col justify-between">
          <div>
            ...
            {failedSearches.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-8">✨ No failed searches yet &mdash; your knowledge base is solid!</p>
            ) : (
